...
namespace df {
	...
	int Sprite::addFrame(Frame new_frame) {
		if (m_frame_count >= m_max_frame_count) return -1;	// Is Sprite full?
	...
	Frame Sprite::getFrame(int frame_number) const {
		if ((frame_number < 0) || (frame_number >= m_frame_count)) return Frame();	// Return empty frame.
	...
	int Sprite::draw(int frame_number, Vector position) const {
		if ((frame_number < 0) || (frame_number >= m_frame_count)) return -1;
